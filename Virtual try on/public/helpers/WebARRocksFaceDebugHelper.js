...
const WebARRocksFaceDebugHelper = (function () {
  ...
  function callbackReady(err, spec) {
    ...
    console.log(
      "INFO in WebARRocksFaceDebugHelper: WebAR.Rocks.face is ready. spec =",
      spec
    );
    ...
    if (_videoElement) {
      console.log(
        "INFO in WebARRocksFaceDebugHelper: video resolution =",
  ...
  function process_faceSlot(detectState, slotIndex) {
    if (detectState.isDetected) {
      ...
      if (_spec.isStabilized) {
        landmarks = get_landmarksStabilizer(slotIndex).update(
  ...
  function init_shps() {
    ...
    _shps.drawPoints = build_shaderProgram(
      shaderVertexSource,
      shaderFragmentSource,
      "DRAWPOINT"
    );
    _shps.drawPoints.uniforms.pointSize = _gl.getUniformLocation(
      _shps.drawPoints.program,
      "pointSize"
    );
  }
