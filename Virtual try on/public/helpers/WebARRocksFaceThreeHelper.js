...
const WebARRocksFaceThreeHelper = (function () {
  ...
  const _defaultSolvePnPObjPointsPositions = {
    // 3d positions, got using Blender in edit mode and opening dev/face.obj
    // the value added as comment is the point indice
    leftEyeCtr: [33.7, 37.9, 45.9], // 6022
    rightEyeCtr: [-33.7, 37.9, 45.9], // 5851
  ...
  function init_PnPSolver(imgPointsLabels, objPointsPositions) {
    ...
    _computePose.imgPointsLMIndices = imgPointsLabels.map(function (label) {
      const ind = _landmarks.labels.indexOf(label);
      if (ind === -1) {
        throw new Error(
          "This neuron network model does not have any point with label=" +
            label
        );
      }
      return ind;
    ...
    _computePose.objPoints = imgPointsLabels.map(function (label) {
  ...
  function init_three(maxFacesDetected) {
    console.log(
      "INFO in WebARRocksFaceThreeHelper - init_three(). Max faces detected = ",
      maxFacesDetected
    );
    ...
    _three.preMatrix.setPosition(
      0.0,
      _spec.translationYZ[0],
      _spec.translationYZ[1]
    );
    _three.preMatrix.scale(
      new THREE.Vector3(1.0, 1.0, 1.0).multiplyScalar(_spec.scale)
    );
  ...
  function update_lightReconstruction(detectState) {
    ...
    let ambIntensity = detectState.lAmb,
      dirIntensity = detectState.lDir;
    ...
    // apply pow:
    ambIntensity = Math.pow(
      ambIntensity,
      _spec.lightReconstructionIntensityPow
    );
    dirIntensity = Math.pow(
      dirIntensity,
      _spec.lightReconstructionIntensityPow
    );

    // apply light intensities:
    _three.ambLight.intensity =
      ambIntensity * _spec.lightReconstructionAmbIntensityFactor;
    _three.dirLight.intensity =
      dirIntensity * _spec.lightReconstructionDirIntensityFactor;
    ...
    const theta = detectState.lTheta,
      phi = detectState.lPhi;
  ...
  function callbackReady(err, spec) {
    ...
    console.log(
      "INFO in WebARRocksFaceThreeHelper: WebAR.Rocks.face is ready. spec =",
      spec
    );
    ...
    console.log(
      "INFO in WebARRocksFaceThreeHelper: video resolution =",
    ...
    if (typeof WebARRocksLMStabilizer === "undefined") {
      ...
      console.warn(
        "WARNING in WebARRocksFaceThreeHelper: cannot find WebARRocksLMStabilizer. Points won't be stabilized"
      );
    } else {
    ...
    update_focals();
    init_PnPSolver(
      _spec.solvePnPImgPointsLabels,
      _spec.solvePnPObjPointsPositions
    );
  ...
  function draw_video() {
    ...
    _gl.uniformMatrix2fv(
      _shps.copyCrop.uniforms.transformMat2,
      false,
      _videoTransformMat2
    );
  ...
  function process_faceSlot(detectState, slotIndex) {
    ...
    if (detectState.isDetected) {
      ...
      if (_landmarksStabilizers === null) {
        ...
        landmarksStabilized = _landmarksStabilizers[slotIndex].update(
          detectState.landmarks,
          that.get_viewWidthCSSPx(),
          that.get_viewHeightCSSPx(),
          detectState.s
        );
      }
  ...
  function compute_pose(landmarks, faceSlot) {
    ...
    const solved = WEBARROCKSFACE.compute_pose(
      objectPoints,
      imgPointsPx,
      _focals[0],
      _focals[1]
  ...
  function init_shps() {
    ...
    _shps.copyCrop = build_shaderProgram(
    ...
    _shps.copyCrop.uniforms.transformMat2 = _gl.getUniformLocation(
      _shps.copyCrop.program,
      "transform"
    );
  }
  ...
  function update_torsoPose(faceFollowerParent, torso) {
    ...
    torso.matrix
      .copy(torso.parent.matrixWorld)
      .invert()
      .multiply(torso.matrixWorld);
  ...
  const that = {
      ...
      if (_spec.videoURL) {
        ...
        return new Promise(function (accept, reject) {
          domVideo.oncanplay = function (e) {
            console.log(
              "INFO in WebARRocksFaceThreeHelper: video file can play"
            );
    ...
    update_solvePnP: function (objPointsPositions, imgPointsLabels) {
      if (objPointsPositions) {
        _spec.solvePnPObjPointsPositions = Object.assign(
          _spec.solvePnPObjPointsPositions,
          objPointsPositions
        );
      }
      _spec.solvePnPImgPointsLabels =
        imgPointsLabels || _spec.solvePnPImgPointsLabels;
      init_PnPSolver(
        _spec.solvePnPImgPointsLabels,
        _spec.solvePnPObjPointsPositions
      );
    ...
    update_threeCamera: function () {
      ...
      console.log(
        "INFO in WebARRocksFaceThreeHelper.update_threeCamera(): vFoV=",
        fov,
        "deg, canvasAspectRatio=",
        canvasAspectRatio
      );
