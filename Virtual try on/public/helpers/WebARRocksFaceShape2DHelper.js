...
const WebARRocksFaceShape2DHelper = (function () {
  ...
  function build_shape(landmarkLabels, shapeSpecsArg, shapeIndex) {
    ...
    return new Promise(function (accept, reject) {
      ...
      const GLSLTexturesSamplers = shapeSpecs.textures.map(function (
        textureSpec
      ) {
        return "uniform sampler2D " + textureSpec.id + ";";
      });
      const GLSLUniforms = shapeSpecs.uniforms.map(function (uniformSpec) {
        const GLSLUniformType = ["float", "vec2", "vec3", "vec4"][
          uniformSpec.value.length - 1
        ];
      ...
      shp.uniforms.samplerVideo = _gl.getUniformLocation(
        shp.program,
        "samplerVideo"
      );
      shp.uniforms.videoUVScale = _gl.getUniformLocation(
        shp.program,
        "videoUVScale"
      );
      shp.uniforms.texturesSamplers = shapeSpecs.textures.map(function (
        textureSpec
      ) {
        return _gl.getUniformLocation(shp.program, textureSpec.id);
      });
      shapeSpecs.uniforms.forEach(function (uniformSpec) {
        shp.uniforms[uniformSpec.name] = _gl.getUniformLocation(
          shp.program,
          uniformSpec.name
        );
      });
      ...
      if (shapeSpecs.interpolations) {
        ...
        const interpolationsSplitted = split_interpolations(
          shapeSpecs.interpolations
        );

        // build interpolated points:
        let interpInd = 0;
        for (let i = 0; i < interpolationsSplitted.length; ++i) {
          const interpPoints = build_interpolation(
            shapeSpecs,
            interpolationsSplitted[i],
            interpInd
          );
      ...
      const points = new Float32Array(
        (pointsCount + interpolatedPointsCount) * 2
      );
      ...
      shapeSpecs.points.forEach(function (label, ind) {
        ...
        if (lmInd === -1) {
          throw new Error(
            "The neural network does not outputs this landmark" + label
          );
        }
      ...
      texturesPromise.then(function (textures) {
        const shapeBuilt = {
          outlines: shapeSpecs.outlines
            ? shapeSpecs.outlines.map(build_outline)
            : [],
  ...
  function build_interpolation(
    ...
    const firstInterpolatedPointInd =
      shapeSpecs.points.length + interpolationInd;
    ...
    for (let i = 0; i < tess.length; i += 3) {
      ...
      for (let i = 0; i < ks.length; ++i) {
        ...
        const nextPt =
          i === ks.length - 1 ? pt1 : firstInterpolatedPointInd + i + 1;
        if (ks[i] > 0) {
    ...
    for (let i = 0; i < ks.length; ++i) {
      ...
      const interpolatedPointInd =
        shapeSpecs.points.length + interpolationInd + i;
  ...
  function draw_shape(landmarksPositions, shape) {
    ...
    shape.interpolatedPoints.forEach(
      compute_interpolation.bind(null, shape.points)
    );
    ...
    shape.textures.forEach(function (glTexture, ind) {
      ...
      _gl.activeTexture(
        [_gl.TEXTURE1, _gl.TEXTURE2, _gl.TEXTURE3, _gl.TEXTURE4][ind]
      );
      _gl.bindTexture(_gl.TEXTURE_2D, glTexture);
    });
  ...
  function apply_outline(pointPositions, outline) {
    ...
    for (let i = 0; i < outline.pointsCount; ++i) {
      ...
      const prevToThis =
        outline.sides[i === 0 ? outline.pointsCount - 1 : i - 1];
  ...
  function compute_interpolation(pointPositions, interpolation) {
    ...
    const m0 = interpolation.m0,
      m1 = interpolation.m1;
    m0[0] =
      pointPositions[2 * interpolation.points[2]] -
      pointPositions[2 * interpolation.points[0]];
    m0[1] =
    ...
    const sizeRef = l1;
    const s0 =
      (Math.sign(interpolation.k) *
        sizeRef *
        interpolation.tangentInfluences[1]) /
      l0;
    ...
    pointPositions[interpolation.ind * 2] =
      p0k * p0x + m0k * m0x + m1k * m1x + p1k * p1x;
    pointPositions[interpolation.ind * 2 + 1] =
      p0k * p0y + m0k * m0y + m1k * m1y + p1k * p1y;
    ...
    return new Promise(function (accept, reject) {
      const initSettings = {
        ...
        callbackReady: function (err, objs) {
          ...
          console.log(
            "INFO in WebARRocksFaceShape2DHelper: WEBARROCKSFACE is initialized"
          );
          _glv = objs.GL;
          ...
          init_shps();
          Promise.all(
            _spec.shapes.map(build_shape.bind(null, objs.landmarksLabels))
