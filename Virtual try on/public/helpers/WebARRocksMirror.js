...
const WebARRocksMirror = (function () {
  ...
  function tweak_material(threeMat, isGlassesBranch) {
    ...
    newMat.onBeforeCompile = function (shaders) {
      ...
      if (isGlassesBranch) {
        const glassesBranchUniforms = {
          uBranchFading: {
            value: new THREE.Vector2(
              _spec.branchFadingZ,
              _spec.branchFadingTransition
            ),
          }, // first value: position (lower -> to the back), second: transition brutality
        ...
        vertexShaderSource =
          "uniform float uBranchBendingAngle, uBranchBendingZ;\n" +
          vertexShaderSource;
        let GLSLBendBranch =
          "float zBranch = max(0.0, uBranchBendingZ-position.z);\n";
        GLSLBendBranch +=
        ...
        vertexShaderSource = insert_GLSLAfter(
          vertexShaderSource,
        ...
        const GLSLcomputeAlpha =
          "gl_FragColor *= smoothstep(uBranchFading.x - uBranchFading.y * 0.5, uBranchFading.x + uBranchFading.y * 0.5, vPosZ);";
        fragmentShaderSource = insert_GLSLAfter(
          fragmentShaderSource,
  ...
  function tweak_faceAccessory(threeFaceAccessory) {
    ...
    threeFaceAccessory.traverse(function (threeNode) {
      ...
      isGlassesBranch =
        isGlassesBranch && mat.name && mat.name.indexOf("frame") !== -1;
  ...
  function build_scene() {
    ...
    if (_spec.envmapURL) {
      ...
      new THREE.RGBELoader(_threeInstances.loadingManager)
        .setDataType(THREE.HalfFloatType)
        .load(_spec.envmapURL, function (texture) {
          _threeInstances.envMap =
    ...
    if (_spec.occluderURL) {
      WebARRocksFaceThreeHelper.add_occluderFromFile(
        _spec.occluderURL,
        null,
        _threeInstances.loadingManager,
        _spec.debugOccluder
      );
    }
    ...
    if (_spec.bloom) {
      ...
      const bloomPass = new THREE.UnrealBloomPass(
        new THREE.Vector2(_spec.canvasThree.width, _spec.canvasThree.height),
    ...
    init: function (spec) {
      return new Promise(function (resolve, reject) {
        ...
        const threeHelperSpec = {
          ...
          lightReconstructionIntensityPow:
            _spec.lightReconstructionIntensityPow,
          lightReconstructionAmbIntensityFactor:
            _spec.lightReconstructionAmbIntensityFactor,
          lightReconstructionDirIntensityFactor:
            _spec.lightReconstructionDirIntensityFactor,
          lightReconstructionTotalIntensityMin:
            _spec.lightReconstructionTotalIntensityMin,
        ...
        if (spec.solvePnPObjPointsPositions) {
          threeHelperSpec.solvePnPObjPointsPositions =
            spec.solvePnPObjPointsPositions;
        }
        if (spec.solvePnPImgPointsLabels) {
          threeHelperSpec.solvePnPImgPointsLabels =
            spec.solvePnPImgPointsLabels;
        }
...
window.WebARRocksMirror = WebARRocksMirror;
